...
def your_tts(text,lang,gender,audio_path,actual_duration,speed=1.0,tts_model="Kokoro TTS",voice_name="af_heart"):
  ...
  if tts_model=="Kokoro TTS":
    tts_path,_,_,_,_=single_tts(text, Language=lang,voice=voice_name, speed=speed,remove_silence=False,keep_silence_up_to=0.05,translate=False)
    if tts_path is None:
        print(f"Kokoro TTS not available, using Microsoft TTS")
  ...
  if actual_duration > 0:
    if tts_duration > actual_duration:
      ...
      if tts_model=="Kokoro TTS":
        tts_path,_,_,_,_=single_tts(text, Language=lang,voice=voice_name, speed=speedup_factor,remove_silence=False,keep_silence_up_to=0.05,translate=False)
        if tts_path is None:
...
class SRTDubbing:
    ...
    def srt_to_dub(self, srt_file_path,dub_save_path,language='English',gender='Male',tts_model="Kokoro TTS",voice_name="af_heart"):
...
def dubbing(srt_file_path,langauge,gender,tts_model="Kokoro TTS",voice_name="af_heart"):
  srt_dubbing = SRTDubbing()
  dub_save_path=get_subtitle_Dub_path(srt_file_path,langauge)
  srt_dubbing.srt_to_dub(srt_file_path,dub_save_path,langauge,gender=gender,tts_model=tts_model,voice_name=voice_name)
...
def subtitle_maker(Audio_or_Video_File, Source_Language, Destination_Language, subtitle_upload=None, Gender='Male',
    ...
    if subtitle_upload is None:
        ...
        default_srt_path = subtitle_upload
        src_lang = Source_Language  # Assume source language is correct
    ...
    return dubb_voice,default_srt_path, new_video,customize_srt_path, word_level_srt_path, shorts_srt_name, text_path,new_video
