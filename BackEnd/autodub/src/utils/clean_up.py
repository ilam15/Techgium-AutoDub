...
_cleanup_lock = logging.INFO # Misused import, just to check if needed. Better to use threading lock if concurrency matters.
import threading
_registry_lock = threading.Lock()

def register_for_cleanup(path: str):
    """Register a file path for automatic cleanup."""
    if not path: return
    with _registry_lock:
        _cleanup_registry.add(os.path.abspath(path))

def unregister_for_cleanup(path: str):
    """Unregister a file path from automatic cleanup."""
    if not path: return
    try:
        abs_path = os.path.abspath(path)
        with _registry_lock:
            if abs_path in _cleanup_registry:
                _cleanup_registry.remove(abs_path)
    except:
        pass
...
def check_disk_space(path: str = ".", min_gb: float = 2.0) -> bool:
    ...
    try:
        ...
        free_gb = free / (1024**3)
        if free_gb < min_gb:
            logger.warning(f"Low disk space: {free_gb:.2f} GB free (Threshold: {min_gb} GB)")
            return False
        return True
    except Exception as e:
        logger.error(f"Failed to check disk space: {e}")
        return True # Assume ok to avoid blocking if check fails

def get_registered_cleanup_files():
    with _registry_lock:
        return list(_cleanup_registry)
