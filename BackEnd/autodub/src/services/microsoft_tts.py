...
from src.utils.lang_data import languages, male_voice_list, female_voice_list
...
def edge_free_tts(chunks_list, speed, voice_name, save_path, translate_text_flag, Language):
    ...
    temp_txt_path = os.path.join(edge_folder, "temp.txt")
...
def edge_tts_pipeline(
    ...
    audio_folder = os.path.join(edge_folder, "audio")
    if not os.path.exists(audio_folder):
        os.makedirs(audio_folder, exist_ok=True)

    temp_mp3 = os.path.join(audio_folder, random_audio_name_generate())
